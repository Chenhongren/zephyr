...
ZTEST(it51xxx_hw_rsa, test_2048_key_bits)
{
	...
	struct device *hw_rsa_device = (struct device *)DEVICE_DT_GET(DT_NODELABEL(hw_rsa));
	static struct it51xxx_hw_rsa_messages msgs;
	static uint8_t output[sizeof(ciphertext_2048)];
	struct it51xxx_hw_rsa_keys keys = {
	...
	ret = it51xxx_hw_rsa_configure(hw_rsa_device, keys);
	zassert_equal(ret, 0, "failed to configure keys");
	msgs.input = plaintext_2048;
	msgs.input_sz = sizeof(plaintext_2048);
	msgs.output = output;
	msgs.output_sz = sizeof(output);
	memset(output, 0, sizeof(output));
	ret = it51xxx_hw_rsa_encrypt(hw_rsa_device, &msgs);
	zassert_equal(ret, 0, "failed to perform encryption");
	ret = memcmp(msgs.output, ciphertext_2048, sizeof(ciphertext_2048));
	zassert_equal(ret, 0, "encrypt error");

	ret = it51xxx_hw_rsa_configure(hw_rsa_device, keys);
	zassert_equal(ret, 0, "failed to configure keys");
	msgs.input = ciphertext_2048;
	msgs.input_sz = sizeof(ciphertext_2048);
	msgs.output = output;
	msgs.output_sz = sizeof(output);
	memset(output, 0, sizeof(output));
	ret = it51xxx_hw_rsa_decrypt(hw_rsa_device, exponent_3, &msgs);
	zassert_equal(ret, 0, "failed to perform decryption");

	ret = memcmp(msgs.output, plaintext_2048, sizeof(plaintext_2048));
...
ZTEST(it51xxx_hw_rsa, test_4096_key_bits)
{
	...
	ret = it51xxx_hw_rsa_configure(hw_rsa_device, keys);
	zassert_equal(ret, 0, "failed to configure keys");
	msgs.input = plaintext_4096;
	msgs.input_sz = sizeof(plaintext_4096);
	msgs.output = output;
	msgs.output_sz = sizeof(output);
	memset(output, 0, sizeof(output));
	ret = it51xxx_hw_rsa_encrypt(hw_rsa_device, &msgs);
	zassert_equal(ret, 0, "failed to perform encryption");
	ret = memcmp(msgs.output, ciphertext_4096, sizeof(ciphertext_4096));
	zassert_equal(ret, 0, "encrypt error");

	ret = it51xxx_hw_rsa_configure(hw_rsa_device, keys);
	zassert_equal(ret, 0, "failed to configure keys");
	msgs.input = ciphertext_4096;
	msgs.input_sz = sizeof(ciphertext_4096);
	msgs.output = output;
	msgs.output_sz = sizeof(output);
	memset(output, 0, sizeof(output));
	ret = it51xxx_hw_rsa_decrypt(hw_rsa_device, exponent_3, &msgs);
	zassert_equal(ret, 0, "failed to perform encryption");
	ret = memcmp(msgs.output, plaintext_4096, sizeof(plaintext_4096));
	zassert_equal(ret, 0, "decrypt error");
}
