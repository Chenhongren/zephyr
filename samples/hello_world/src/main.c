...
/* it51xxx spi driver doesn't support dual/quad mode */
#define TEST_QUAD_MODE		0
#define TEST_DUAL_MODE		0
#define TEST_LINE_MODE		1
#define TEST_FLASH_ACCESS	1
#define TEST_PM_CHECK		1
#define TEST_RX_ONLY		1
#define TEST_TX_ONLY		1

#define PLL_FREQ_SELECTIION 0

#define SPI_FLASH_TEST_REGION_OFFSET 0x0

#if TEST_PM_CHECK
#define SPI_FLASH_SECTOR_SIZE 0x1000 * 3 /* base 0x3000 = 12288 bytes */
#define PM_CHECK_STACK_SIZE 4096 * 20
K_THREAD_STACK_DEFINE(pm_check_stack, PM_CHECK_STACK_SIZE);
#else
...
static void pm_check_cb(void *p1, void *p2, void *p3)
{
	ARG_UNUSED(p1);
	ARG_UNUSED(p2);
	ARG_UNUSED(p3);
...
int main(void)
{
#if PLL_FREQ_SELECTIION
	uint32_t clk = MHZ(48); /* pll frequency */
#else
	uint32_t clk = KHZ(9200); /* ec frequency */
#endif
	int ret;

	spi_cfg.operation |= SPI_LINES_SINGLE;
	spi_cfg.frequency = clk / 16;
...
#endif
		if (i == 0) {
			spi_cfg.frequency = clk;
		} else {
			spi_cfg.frequency = clk / (i * 2);
		}
	...
	ret = read_jedec_id(spi_device);
	if (ret) {
		LOG_ERR("failed to test line mode");
		return ret;
	}
	spi_cfg.operation &= ~(SPI_MODE_CPHA | SPI_MODE_CPOL);
#endif
...
#if TEST_PM_CHECK
K_THREAD_DEFINE(blink0_id, PM_CHECK_STACK_SIZE, pm_check_cb, NULL, NULL, NULL,
				7, 0, 0);
#endif
