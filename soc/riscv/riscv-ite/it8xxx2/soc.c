...
static int chip_change_pll(void)
{
	...
	if (IS_ENABLED(CONFIG_USB_DC_IT82XX2) && IS_ENABLED(CONFIG_SOC_IT8XXX2_INT_32K)) {
		chip_configure_pll(&pll_configuration[PLL_FREQ_96M]);

		if (IT8XXX2_ECPM_PFACC2R != AUTO_CAL_ENABLE_AND_START ||
		    (IT8XXX2_ECPM_LCOTF2 & LCO_SC_FACTOR_MASK) != LCO_SC_FACTOR(2)) {
			IT8XXX2_ECPM_PFACC0R &= ~PLL_FREQ_AUTO_CAL_EN;
			if (IS_ENABLED(CONFIG_SOC_IT8XXX2_EC_BUS_24MHZ)) {
				IT8XXX2_ECPM_HIGH_BYTE_FOR_LCVCO_CAL =
					DIV_ROUND_UP(IT8XXX2_ECPM_HIGH_BYTE_FOR_LCVCO_CAL, 3);
				IT8XXX2_ECPM_LOW_BYTE_FOR_LCVCO_CAL =
					DIV_ROUND_UP(IT8XXX2_ECPM_LOW_BYTE_FOR_LCVCO_CAL, 3);
			}
