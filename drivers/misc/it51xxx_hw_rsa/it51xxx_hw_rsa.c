...
static int it51xxx_start_rsa_calculation(const struct device *dev)
{
	...
	/* wait for completion */
	ret = k_sem_take(&data->rsa_cal_done, K_MSEC(CONFIG_IT51XXX_RSA_CALCULATION_TIMEOUT_MS));
	...
	sys_write8(sys_read8(base + RSA42_STATUS) | RSA_CALCULATION_DONE, base + RSA42_STATUS);
...
static void rsa_complete(void *user_data, uint8_t status)
{
	struct it51xxx_hw_rsa_data *data = user_data;

	if (status & RSA_BUSY) {
		LOG_WRN("rsa completed but busy, sts %#x", status);
	}
...
static int it51xxx_hw_rsa_init(const struct device *dev)
{
	...
	it51xxx_hwcrypto_register_rsa_cb(config->mfd, rsa_complete, data);
