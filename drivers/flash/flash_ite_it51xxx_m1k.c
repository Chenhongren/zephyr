					...
					const uintptr_t in, void *out)
{
	...
	if (request != NULL) {
		...
		for (int path = 0; path < IT51XXX_PROTECT_PATH_COUNT; path++) {
			if (!IS_BIT_SET(request->path, path)) {
				continue;
			}

			for (uint32_t offset = 0; offset < request->size; offset += sector_size) {
				uint32_t cur = request->addr + offset;
				uint8_t bit = (cur / sector_size) & SECTOR_BIT_MASK;
				uint8_t reg_val;

				reg = eflash_protect_reg(path, write, cur, block_size);
				reg_val = sys_read8(reg);
				WRITE_BIT(reg_val, bit, request->is_protected);
	...
	if (result != NULL) {
		...
		for (int path = 0; path < IT51XXX_PROTECT_PATH_COUNT && result->is_protected;
		     path++) {
			if (!IS_BIT_SET(result->path, path)) {
				continue;
			}
			for (uint32_t offset = 0; offset < result->size; offset += sector_size) {
				uint32_t cur = result->addr + offset;
				uint8_t bit = (cur / sector_size) & SECTOR_BIT_MASK;

				reg = eflash_protect_reg(path, write, cur, block_size);
				if ((sys_read8(reg) & BIT(bit)) == 0) {
