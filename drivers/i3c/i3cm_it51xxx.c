...
static int it51xxx_i3cm_ibi_enable(const struct device *dev, struct i3c_device_desc *target)
{
	...
	if (CONFIG_I3C_IBI_MAX_PAYLOAD_SIZE <= IT51XXX_AUTO_IBI_MAX_PAYLOAD_SIZE) {
		for (uint8_t i = 0; i < CONFIG_IT51XXX_AUTO_IBI_MAX_TARGETS; i++) {
			if (AUTO_IBI_RESP_ADDR_GET(sys_read8(cfg->base + auto_ibi_base[i] +
							     I3CM60_IBI_AUTO_RESP_0_CTRL_0)) != 0) {
				continue;
			}

			sys_write8(AUTO_IBI_RESP_ADDR_PREP(target->dynamic_addr),
				   cfg->base + auto_ibi_base[i] + I3CM60_IBI_AUTO_RESP_0_CTRL_0);
			sys_write8(AUTO_IBI_RESP_PAYLOAD_SIZE(IT51XXX_AUTO_IBI_MAX_PAYLOAD_SIZE),
				   cfg->base + auto_ibi_base[i] + I3CM61_IBI_AUTO_RESP_0_CTRL_1);
			/* enable auto ibi one-shot mode */
			sys_write8(IBI_ONE_SHOT_AUTO_RESP_EN,
				   cfg->base + auto_ibi_base[i] + I3CM62_IBI_AUTO_RESP_0_CTRL_2);
			break;
		}
	}
#endif /* CONFIG_IT51XXX_AUTO_IBI_MAX_TARGETS */
...
static int it51xxx_i3cm_ibi_disable(const struct device *dev, struct i3c_device_desc *target)
{
	...
	if (CONFIG_I3C_IBI_MAX_PAYLOAD_SIZE <= IT51XXX_AUTO_IBI_MAX_PAYLOAD_SIZE) {
		for (uint8_t i = 0; i < CONFIG_IT51XXX_AUTO_IBI_MAX_TARGETS; i++) {
			if (AUTO_IBI_RESP_ADDR_GET(sys_read8(cfg->base + auto_ibi_base[i] +
							     I3CM60_IBI_AUTO_RESP_0_CTRL_0)) ==
			    target->dynamic_addr) {
				sys_write8(AUTO_IBI_RESP_ADDR_PREP(0),
					   cfg->base + auto_ibi_base[i] +
						   I3CM60_IBI_AUTO_RESP_0_CTRL_0);
				sys_write8(0, cfg->base + auto_ibi_base[i] +
