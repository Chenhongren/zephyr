				       ...
				       struct i3c_config_target *cfg_target)
{
	...
	if (!cfg_target->pid_random) {
		...
		cfg_target->pid =
			...
			((uint32_t)sys_read8(cfg->base + I3CS6F_PRAT_NUMBER_3) << 24) |
			((uint32_t)sys_read8(cfg->base + I3CS6E_PRAT_NUMBER_2) << 16) |
			((uint32_t)sys_read8(cfg->base + I3CS6D_PRAT_NUMBER_1) << 8) |
			((uint32_t)sys_read8(cfg->base + I3CS6C_PRAT_NUMBER_0));
	} else {
		sys_write8(sys_read8(cfg->base + I3CS05_CONFIG_1) | ID_RANDOM,
			   cfg->base + I3CS05_CONFIG_1);
