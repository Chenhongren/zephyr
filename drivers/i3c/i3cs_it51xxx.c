...
static void it51xxx_i3cs_prepare_tx_fifo(const struct device *dev, uint8_t *buf, uint16_t len)
{
	const struct it51xxx_i3cs_config *cfg = dev->config;
	struct it51xxx_i3cs_data *data = dev->data;
	uint16_t tx_count;
					...
					uint8_t hdr_mode)
{
	...
	it51xxx_i3cs_prepare_tx_fifo(dev, buf, len);
...
static int it51xxx_i3cs_target_ibi_raise(const struct device *dev, struct i3c_ibi *request)
{
	...
	case I3C_IBI_TARGET_INTR:
		...
		k_sem_take(&data->event_lock_sem, K_FOREVER);
		if (request->payload_len == 0) {
			...
			if (request->payload_len > 1) {
				/* set pending read notification */
				it51xxx_i3cs_prepare_tx_fifo(dev, &request->payload[1],
	...
	if (it51xxx_i3cs_wait_to_complete(dev) != 0) {
		...
		k_sem_give(&data->event_lock_sem);
	}
out:
	k_mutex_unlock(&data->lock);
	return ret;
...
static int it51xxx_i3cs_init(const struct device *dev)
{
	...
	k_sem_init(&data->event_lock_sem, 1, 1);
...
static void it51xxx_i3cs_process_rx_fifo(const struct device *dev, const bool ccc)
{
	...
	if (rx_direct_mode_is_enabled(dev)) {
		...
		if (dfifo_status & I3CS_DIRECT_RX_DONE) {
			memcpy(rx_buf, data->fifo.rx_data, byte_count);
	...
	if (!ccc) {
		LOG_HEXDUMP_DBG(rx_buf, byte_count, "isr: rx:");
#ifdef CONFIG_I3C_TARGET_BUFFER_MODE
		if (target_cb && target_cb->buf_write_received_cb) {
			target_cb->buf_write_received_cb(data->target_config, rx_buf, byte_count);
		}
#endif /* CONFIG_I3C_TARGET_BUFFER_MODE */
	} else {
		LOG_HEXDUMP_WRN(rx_buf, byte_count, "isr: unhandled ccc:");
	}
}
...
#define IT51XXX_I3CS_INIT(n)                                                                       \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), it51xxx_i3cs_isr,           \
