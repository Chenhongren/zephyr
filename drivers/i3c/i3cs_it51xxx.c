...
static inline void set_pid_part_number(const struct device *dev, const uint64_t pid)
{
	const struct it51xxx_i3cs_config *cfg = dev->config;
	uint32_t pid_part_number = GET_PART_NUMBER(pid);

	sys_write8(sys_read8(cfg->base + I3CS05_CONFIG_1) | ID_RANDOM, cfg->base + I3CS05_CONFIG_1);
	sys_write8(BYTE_0(pid_part_number), cfg->base + I3CS6C_PRAT_NUMBER_0);
	sys_write8(BYTE_1(pid_part_number), cfg->base + I3CS6D_PRAT_NUMBER_1);
	sys_write8(BYTE_2(pid_part_number), cfg->base + I3CS6E_PRAT_NUMBER_2);
	sys_write8(BYTE_3(pid_part_number), cfg->base + I3CS6F_PRAT_NUMBER_3);
}
...
static int it51xxx_i3cs_configure(const struct device *dev, enum i3c_config_type type, void *config)
{
	...
	set_pid_part_number(dev, cfg_target->pid);
	LOG_INST_INF(cfg->log, "pid: %#llx", cfg_target->pid);
	...
	/* TODO
	 * THINK: NEED TO SET DYNAMIC ADDRESS OR DYNAMIC ADDRESS
	 * SHOULD BE SET BY CONTROLLER(DAA PROCESS)
	 *
	 * cfg_target->dynamic_addr
	 */
...
static int it51xxx_i3cs_init(const struct device *dev)
{
	...
	/* set pid, bcr and dcr */
	LOG_INST_INF(cfg->log, "pid: %#llx", config_target->pid);
	if (config_target->pid_random) {
		set_pid_part_number(dev, config_target->pid);
	}
...
/* TODO: NEED TO CHECK
 * PID[31:0] : part number and instance is ?
 */
