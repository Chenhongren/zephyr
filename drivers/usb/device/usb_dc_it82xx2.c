...
static void it82xx2_handler_setup(uint8_t idx, uint8_t ep_ctrl)
{
	struct usb_it82xx2_regs *const usb_regs =
		(struct usb_it82xx2_regs *)it82xx2_get_usb_regs();
	struct it82xx2_usb_ep_regs *ep_regs = usb_regs->usb_ep_regs;
	...
	if (ep_ctrl & EP_SEND_STALL) {
		ep_regs[idx].ep_ctrl.fields.send_stall_bit = 0;
	...
	ep_regs[idx].ep_ctrl.fields.outdata_sequence_bit = 1;
	udata0.ep_data[idx].cb_out(USB_EP_DIR_OUT, USB_DC_EP_SETUP);

	if (udata0.no_data_ctrl) {
		ep_regs[EP0].ep_ctrl.fields.ready_bit = 1;
		udata0.no_data_ctrl = false;
	}
}

static void it82xx2_handler_in(uint8_t idx, uint8_t ep_ctrl)
{
	struct usb_it82xx2_regs *const usb_regs =
		(struct usb_it82xx2_regs *)it82xx2_get_usb_regs();
	struct it82xx2_usb_ep_regs *ep_regs = usb_regs->usb_ep_regs;
	...
	if (idx != EP0) {
		if (!udata0.fifo_ready[idx - 1]) {
			return;
		}

		ep_idx = (epn_ext_ctrl[idx].epn_ext_ctrl2 >> 4) & 0x0F;
		if (ep_idx == EP0 || udata0.ep_data[ep_idx].ep_status != EP_CONFIG_IN) {
			return;
		}
		udata0.fifo_ready[idx - 1] = false;
	} else {
		ep_idx = EP0;
		/* stall send check */
		if (ep_ctrl & EP_SEND_STALL) {
			ep_regs[EP0].ep_ctrl.fields.send_stall_bit = 0;
			udata0.st_state = STALL_SEND;
		...
		if (udata0.st_state == DOUT_ST) {
			/* setup -> out(data) -> in(status) */
			udata0.st_state = STATUS_ST;
		} else if (udata0.ep_data[EP0].remaining == 0 && udata0.st_state == SETUP_ST) {
			/* setup -> in(status) */
			udata0.st_state = STATUS_ST;
		} else {
			/* setup -> in(data) */
	...
	it82xx2_usb_set_ep_ctrl(ep_idx, EP_DATA_SEQ_INV, true);
...
static void it82xx2_handler_out(uint8_t idx)
{
	struct usb_it82xx2_regs *const usb_regs =
		(struct usb_it82xx2_regs *)it82xx2_get_usb_regs();
	struct it82xx2_usb_ep_regs *ep_regs = usb_regs->usb_ep_regs;
	...
	if (idx != EP0) {
		...
		ep_idx = (epn_ext_ctrl[idx].epn_ext_ctrl2 >> 4) & 0x0F;
		if (ep_idx == EP0 || udata0.ep_data[ep_idx].ep_status != EP_CONFIG_OUT) {
			return;
		}
		udata0.fifo_ready[idx - 1] = false;
	} else {
	...
	if (idx == EP0) {
		...
		if (it82xx2_check_setup_following_out()) {
			...
			udata0.now_token = SETUP_TOKEN;
			udata0.st_state = SETUP_ST;
			ep_regs[EP0].ep_ctrl.fields.outdata_sequence_bit = 1;
...
static void it82xx2_usb_dc_trans_done(void)
{
	...
	for (uint8_t idx = EP0; idx < EP4; idx++) {
		...
		if ((ep_ctrl & ENDPOINT_EN) && !(ep_ctrl & ENDPOINT_RDY)) {
			...
			case DC_SETUP_TRANS:
				if (idx == EP0) {
					it82xx2_handler_setup(idx, ep_ctrl);
				}
				break;
			case DC_IN_TRANS:
				it82xx2_handler_in(idx, ep_ctrl);
				break;
			case DC_OUTDATA_TRANS:
				it82xx2_handler_out(idx);
				break;
			default:
				break;
			}
...
int usb_dc_reset(void)
{
	...
	for (uint8_t idx = EP1; idx < EP4; idx++) {
	...
	ep_regs[EP0].ep_ctrl.value = BIT(0);
	usb_regs->dc_address = DC_ADDR_NULL;
...
int usb_dc_ep_configure(const struct usb_dc_ep_cfg_data *const cfg)
{
	...
	if (IT8XXX2_IS_EXTEND_ENDPOINT(ep_idx) && in) {
...
int usb_dc_ep_set_callback(const uint8_t ep, const usb_dc_ep_callback cb)
{
	...
	if (!udata0.attached || ep_idx >= MAX_NUM_ENDPOINTS) {
		LOG_DBG("Not attached / Invalid Endpoint: 0x%X", ep_idx);
		return -EINVAL;
	}
...
int usb_dc_ep_enable(const uint8_t ep)
{
	...
	it82xx2_usb_set_ep_ctrl(ep_idx, EP_ENABLE, true);
	LOG_DBG("endpoint 0x%x is enabled", ep_idx);
...
int usb_dc_ep_disable(uint8_t ep)
{
	...
	it82xx2_usb_set_ep_ctrl(ep_idx, EP_ENABLE, false);
	LOG_DBG("endpoint 0x%x is disabled", ep_idx);
			...
			uint32_t *read_bytes)
{
	...
	if (ep_idx == EP0) {
		...
		if (udata0.now_token == SETUP_TOKEN) {
			if (rx_fifo_len == 0) {
				LOG_DBG("[setup token] length 0, reset to 8");
				rx_fifo_len = 8;
			}
			if (rx_fifo_len != 8) {
				LOG_DBG("[setup token] length: %d", rx_fifo_len);
				/* clear rx fifo */
				ff_regs[EP0].ep_rx_fifo_ctrl = FIFO_FORCE_EMPTY;
				return -EIO;
			}
