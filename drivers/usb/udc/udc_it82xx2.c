...
#define IT8XXX2_IS_EXTEND_ENDPOINT(n) (USB_EP_GET_IDX(n) >= EP4)
...
static int it82xx2_usb_extend_ep_ctrl(const struct device *dev, uint8_t ep, enum it82xx2_ep_ctrl ctrl, bool enable)
{
	...
	case EP_STALL_SEND:
		idx = (ep_idx - EP4) / 2;
		if (ep_idx % 2) {
			epn0n1_ext_ctrl[idx].fields.epn1_send_stall_bit = enable;
		} else {
			epn0n1_ext_ctrl[idx].fields.epn0_send_stall_bit = enable;
		}
		break;
	case EP_STALL_CHECK:
		idx = (ep_idx - EP4) / 2;
		if (ep_idx % 2) {
			return epn0n1_ext_ctrl[idx].fields.epn1_send_stall_bit;
		} else {
			return epn0n1_ext_ctrl[idx].fields.epn0_send_stall_bit;
		}
		break;
	case EP_IOS_ENABLE:
		idx = (ep_idx - EP4) / 2;
		if (ep_idx % 2) {
			epn0n1_ext_ctrl[idx].fields.epn1_iso_enable_bit = enable;
		} else {
			epn0n1_ext_ctrl[idx].fields.epn0_iso_enable_bit = enable;
		}
		break;
	case EP_DATA_SEQ_1:
		idx = (ep_idx - EP4) / 2;
		if (ep_idx % 2) {
			epn0n1_ext_ctrl[idx].fields.epn1_outdata_sequence_bit = enable;
		} else {
			epn0n1_ext_ctrl[idx].fields.epn0_outdata_sequence_bit = enable;
		}
		break;
	case EP_DATA_SEQ_INV:
		if (!enable) {
			break;
		}
		idx = (ep_idx - EP4) / 2;
		if (ep_idx % 2) {
			if (epn0n1_ext_ctrl[idx].fields.epn1_outdata_sequence_bit) {
				epn0n1_ext_ctrl[idx].fields.epn1_outdata_sequence_bit = 0;
			} else {
				epn0n1_ext_ctrl[idx].fields.epn1_outdata_sequence_bit = 1;
			}
		} else {
			if (epn0n1_ext_ctrl[idx].fields.epn0_outdata_sequence_bit) {
				epn0n1_ext_ctrl[idx].fields.epn0_outdata_sequence_bit = 0;
			} else {
				epn0n1_ext_ctrl[idx].fields.epn0_outdata_sequence_bit = 1;
			}
		}
		break;
	case EP_IN_DIRECTION_SET:
		idx = ((ep_idx - EP4) % 3) + 1;
		if (((ep_idx - EP4) / 3 == 0)) {
			ext_ctrl[idx].epn_ext_ctrl1.fields.epn0_direction_bit = enable;
		} else if (((ep_idx - EP4) / 3 == 1)) {
			ext_ctrl[idx].epn_ext_ctrl1.fields.epn3_direction_bit = enable;
		} else if (((ep_idx - EP4) / 3 == 2)) {
			ext_ctrl[idx].epn_ext_ctrl1.fields.epn6_direction_bit = enable;
		} else if (((ep_idx - EP4) / 3 == 3)) {
			ext_ctrl[idx].epn_ext_ctrl1.fields.epn9_direction_bit = enable;
		}
		break;
	case EP_ENABLE:
		idx = ((ep_idx - EP4) % 3) + 1;
		if (((ep_idx - EP4) / 3 == 0)) {
			ext_ctrl[idx].epn_ext_ctrl1.fields.epn0_enable_bit = enable;
		} else if (((ep_idx - EP4) / 3 == 1)) {
			ext_ctrl[idx].epn_ext_ctrl1.fields.epn3_enable_bit = enable;
		} else if (((ep_idx - EP4) / 3 == 2)) {
			ext_ctrl[idx].epn_ext_ctrl1.fields.epn6_enable_bit = enable;
		} else if (((ep_idx - EP4) / 3 == 3)) {
			ext_ctrl[idx].epn_ext_ctrl1.fields.epn9_enable_bit = enable;
		}
		break;
	case EP_READY_ENABLE:
		idx = ((ep_idx - EP4) % 3) + 1;
		(enable) ? (ext_ctrl[idx].epn_ext_ctrl2 |= BIT((ep_idx - EP4) / 3))
			 : (ext_ctrl[idx].epn_ext_ctrl2 &= ~BIT((ep_idx - EP4) / 3));
		break;
	default:
...
static int it82xx2_usb_ep_ctrl(const struct device *dev, uint8_t ep, enum it82xx2_ep_ctrl ctrl, bool enable)
{
	const struct usb_it82xx2_config *config = dev->config;
	struct usb_it82xx2_regs *const usb_regs = config->base;
...
static int it82xx2_usb_set_ep_ctrl(const struct device *dev, uint8_t ep, enum it82xx2_ep_ctrl ctrl, bool enable)
{
	const uint8_t ep_idx = USB_EP_GET_IDX(ep);
	int ret = 0;
...
/**
 * @brief Control it82xx2 usb fifo registers
 *
 * Control fifo registers before writing/reading data to/from FIFO.
 *
 * @param ep The endpoint that would like to select FIFO_n as data buffer
 */
static int it82xx2_usb_fifo_ctrl(const struct device *dev, uint8_t ep)
{
	...
	if (ep_idx == EP0) {
		LOG_ERR("Invaild endpoint 0x%x", ep);
		ret = -EINVAL;
	} else {
		...
		if (USB_EP_DIR_IS_IN(ep)) {
			if (ep_idx < EP8) {
				...
				ep_fifo_ctrl[fifon_ctrl + 1] = BIT(ep_idx - EP8);
			}
			      ...
			      struct udc_ep_config *const cfg)
{
	...
	if (IT8XXX2_IS_EXTEND_ENDPOINT(ep_idx)) {
		...
		ep_regs[ep_fifo].ep_ctrl.fields.enable_bit = 1;
	}
...
static int it82xx2_xfer_in_data(const struct device *dev, uint8_t ep, struct net_buf *buf)
{
	...
	len = MIN(buf->len, config->ep_cfg_in[ep_idx].caps.mps);
	...
	if (ep_idx != EP0) {
		priv->fifo_ready[ep_fifo - 1] = true;
	}
...
static int it82xx2_xfer_out_data(const struct device *dev, uint8_t ep, struct net_buf *buf)
{
	...
	if (ep_regs[ep_fifo].ep_status & EP_STATUS_ERROR) {
		LOG_WRN("endpoint%d error status 0x%02x", ep_idx, ep_regs[ep_fifo].ep_status);
		return -EINVAL;
	}

	len = (uint16_t)ff_regs[ep_fifo].ep_rx_fifo_dcnt_lsb +
	...
	for (int idx = 0; idx < len; idx++) {
			data_ptr[idx] = ff_regs[ep_fifo].ep_rx_fifo_data;
	}
...
static int work_handler_xfer_continue(const struct device *dev, uint8_t ep, struct net_buf *buf) {
	...
	if (USB_EP_DIR_IS_OUT(ep)) {
		ret = it82xx2_xfer_out_data(dev, ep, buf);
	} else {
...
static int work_handler_xfer_next(const struct device *dev, uint8_t ep)
{
	...
	int ret = work_handler_xfer_continue(dev, ep, buf);
	if (!ret) {
				   ...
				   const size_t length)
{
	...
	it82xx2_usb_set_ep_ctrl(dev, EP0, EP_READY_ENABLE, true);
...
static inline int work_handler_setup(const struct device *dev, uint8_t ep)
{
	...
	if (udc_ctrl_stage_is_status_out(dev)) {
		buf = udc_buf_get(dev, EP0);
		if (buf == NULL) {
			LOG_ERR("Missing out(status) buffer");
		}
	}
...
static bool it82xx2_fake_token(const struct device *dev, uint8_t ep, uint8_t token_type)
{
	...
	if (ep_idx == EP0) {
		switch(token_type) {
		case DC_IN_TRANS:
			is_fake = !udc_ctrl_stage_is_data_in(dev) && !udc_ctrl_stage_is_status_in(dev) && !udc_ctrl_stage_is_no_data(dev);
			break;
		case DC_OUTDATA_TRANS:
			is_fake = !udc_ctrl_stage_is_data_out(dev) && !udc_ctrl_stage_is_status_out(dev);
			break;
		default:
		...
		if (!priv->fifo_ready[ep_fifo - 1]) {
			is_fake = true;
		} else {
			priv->fifo_ready[ep_fifo - 1] = false;
			is_fake = false;
		}
	}
...
static void it82xx2_usb_xfer_done(const struct device *dev)
{
	...
	for (int idx = EP0; idx < EP4; idx++) {
		uint8_t ep_ctrl = ep_regs[idx].ep_ctrl.value;
		...
		if ((ep_ctrl & ENDPOINT_EN) && !(ep_ctrl & ENDPOINT_RDY)) {
			...
			switch (ep_regs[idx].ep_transtype_sts & DC_ALL_TRANS) {
			case DC_SETUP_TRANS:
				/* SETUP transaction done */
				if (config->ep_cfg_out[ep].caps.control == 1) {
					if (it82xx2_usb_set_ep_ctrl(dev, ep, EP_STALL_CHECK, true)) {
						it82xx2_usb_set_ep_ctrl(dev, ep, EP_STALL_SEND, false);
						ff_regs[ep_idx].ep_rx_fifo_ctrl = FIFO_FORCE_EMPTY;
			...
			case DC_IN_TRANS:
				...
				if (idx == EP0) {
					...
					ep_idx = (epn_ext_ctrl[idx].epn_ext_ctrl2 & COMPLETED_TRANS) >> 4;
					if (ep_idx == EP0) {
						break;
					} else {
...
static void suspended_handler(struct k_work *item)
{
	...
	struct it82xx2_priv_data *priv =
		CONTAINER_OF(dwork, struct it82xx2_priv_data, suspended_work);
