...
K_MSGQ_DEFINE(evt_msgq, sizeof(struct it82xx2_ep_event), 8, sizeof(uint32_t));
...
static int it82xx2_usb_extend_ep_ctrl(const struct device *dev, uint8_t ep,
				      enum it82xx2_ep_ctrl ctrl, bool enable)
{
...
static void it8xxx2_usb_dc_wuc_init(const struct device *dev)
{
	...
	it8xxx2_wuc_set_polarity(config->wuc_list[0].wucs, config->wuc_list[0].mask,
...
static int it82xx2_xfer_in_data(const struct device *dev, uint8_t ep, struct net_buf *buf)
{
	...
	if (ep_idx == EP0) {
		...
		k_sem_take(&priv->fifo_sem[ep_fifo - 1], K_FOREVER);
...
static int work_handler_xfer_continue(const struct device *dev, uint8_t ep, struct net_buf *buf)
{
	...
	if (USB_EP_DIR_IS_OUT(ep)) {
		...
		it82xx2_usb_set_ep_ctrl(dev, ep_idx, EP_READY_ENABLE, true);
		if (IT8XXX2_IS_EXTEND_ENDPOINT(ep_idx)) {
			it82xx2_usb_set_ep_ctrl(dev, ep_fifo, EP_READY_ENABLE, true);
		}
		if (ep_idx != EP0) {
			priv->fifo_ready[ep_fifo - 1] = true;
		}
...
static inline int work_handler_in(const struct device *dev, uint8_t ep)
{
	...
	struct net_buf *buf;
	uint8_t ep_fifo = ep_fifo_res[USB_EP_GET_IDX(ep) % FIFO_NUM];
	int err = 0;
	...
	if (ep != USB_CONTROL_EP_IN) {
		k_sem_give(&priv->fifo_sem[ep_fifo - 1]);
	}
	...
	while (true) {
		...
		if (evt.ep != USB_CONTROL_EP_OUT && !udc_ep_is_busy(evt.dev, evt.ep)) {
			if (work_handler_xfer_next(evt.dev, evt.ep) == 0) {
				udc_ep_set_busy(dev, evt.ep, true);
			}
		}
	}
}
...
static void it82xx2_usb_xfer_done(const struct device *dev)
{
	...
	for (int fifo_idx = 0; fifo_idx < 4; fifo_idx++) {
		...
		if ((ep_ctrl & ENDPOINT_EN) && !(ep_ctrl & ENDPOINT_RDY)) {
			...
			case DC_SETUP_TRANS:
				...
				if (fifo_idx == 0) {
					...
					if (it82xx2_usb_set_ep_ctrl(dev, ep, EP_STALL_CHECK,
								    true)) {
						it82xx2_usb_set_ep_ctrl(dev, ep, EP_STALL_SEND,
									false);
						ff_regs[ep_idx].ep_rx_fifo_ctrl = FIFO_FORCE_EMPTY;
...
static int it82xx2_init(const struct device *dev)
{
	...
	ret = udc_ep_enable_internal(dev, USB_CONTROL_EP_OUT, USB_EP_TYPE_CONTROL,
				     config->ep_cfg_out[EP0].caps.mps, 0);
	if (ret) {
