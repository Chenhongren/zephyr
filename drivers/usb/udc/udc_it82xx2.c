				      ...
				      enum it82xx2_ep_ctrl ctrl, bool enable)
{
	...
	if ((ctrl == EP_IN_DIRECTION_SET) || (ctrl == EP_ENABLE) || (ctrl == EP_READY_ENABLE)) {
		idx = ((ep_idx - 4) % 3) + 1;
	} else {
		idx = (ep_idx - 4) / 2;
	}
...
static int it82xx2_ep_enable(const struct device *dev, struct udc_ep_config *const cfg)
{
	...
	if (IT8XXX2_IS_EXTEND_ENDPOINT(ep_idx)) {
		uint8_t ep_fifo = ep_fifo_res[ep_idx % SHARED_FIFO_NUM];

		it82xx2_usb_set_ep_ctrl(dev, ep_fifo, EP_ENABLE, true);
	}
...
static int it82xx2_xfer_in_data(const struct device *dev, uint8_t ep, struct net_buf *buf)
{
	...
	uint8_t ep_fifo = (ep_idx > 0) ? (ep_fifo_res[ep_idx % SHARED_FIFO_NUM]) : 0;
	size_t len;
...
static bool it82xx2_fake_token(const struct device *dev, uint8_t ep, uint8_t token_type)
{
	...
	if (ep_idx == 0) {
		...
		ep_fifo = ep_fifo_res[ep_idx % SHARED_FIFO_NUM];
...
static inline int work_handler_in(const struct device *dev, uint8_t ep)
{
	...
	struct net_buf *buf;
	uint8_t ep_fifo = ep_fifo_res[USB_EP_GET_IDX(ep) % SHARED_FIFO_NUM];
	int err = 0;
		...
		if ((ep_ctrl & ENDPOINT_EN) && !(ep_ctrl & ENDPOINT_RDY)) {
				...
				if (fifo_idx == 0) {
					...
					if (it82xx2_usb_set_ep_ctrl(dev, ep, EP_STALL_CHECK,
						...
						it82xx2_usb_set_ep_ctrl(dev, ep, EP_STALL_SEND,
									false);
						ff_regs[ep_idx].ep_rx_fifo_ctrl = FIFO_FORCE_EMPTY;
						LOG_ERR("Cleared stall bit and rx fifo");
						break;
					}
			...
			case DC_IN_TRANS:
				...
				if (fifo_idx == 0) {
					...
					/* Check the IN transaction is STALL */
					if (ep_regs[fifo_idx].ep_status & DC_STALL_SENT) {
						break;
					}
				} else {
