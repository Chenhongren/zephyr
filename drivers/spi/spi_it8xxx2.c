...
static int spi_it8xxx2_configure(const struct device *dev, const struct spi_config *spi_cfg)
{
	...
	if (SPI_MODE_GET(spi_cfg->operation) & SPI_MODE_CPHA) {
		LOG_ERR("Unsupported the spi mode 1 and mode 3");
		return -ENOTSUP;
	}
...
static void it8xxx2_spi_isr(const void *arg)
{
	...
	reg_val = sys_read8(cfg->base + SPI0C_INT_STS);
	if (reg_val & SPI_DMA_RBUF_0_FULL) {
		LOG_INF("Triggered dma ring buffer 0 full interrupt");
		sys_write8(SPI_DMA_RBUF_0_FULL, cfg->base + SPI0C_INT_STS);
	}

	if (reg_val & SPI_DMA_RBUF_1_FULL) {
		LOG_INF("Triggered dma ring buffer 1 full interrupt");
		sys_write8(SPI_DMA_RBUF_1_FULL, cfg->base + SPI0C_INT_STS);
	}

	if (reg_val & SPI_CMDQ_BUS_END) {
		sys_write8(SPI_CMDQ_BUS_END, cfg->base + SPI0C_INT_STS);
